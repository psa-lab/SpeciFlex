...
int main (int argc, char **argv)
{
  if ( argc < 7 )
    {
      printf ("%s <protein ID> <protein pdb> <trajectory> <ligand pdb> <grid spacing> <shell radius> <probe radius>\n", argv[0] );
      return -1;
    }
  ...
  grid_spacing = (float) atof (argv[5]);
  shell_radius = (float) atof (argv[6]);
  tol = (float) atof (argv[7]);
